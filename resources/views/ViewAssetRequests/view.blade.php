            ...
            <ul class="navbar-nav ml-auto" >
                ...
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                        <a class="dropdown-item" href="ViewAssetRequests.view"><span class="item-text">Assets Requests</span></a>
                ...
                <div class="row" style="margin-top: -100px;">
                    <div class="col-lg-6">
                        <div class="text-container">
                            <h1><span class="turquoise">List of all</span> Received Asset Requests</h1>
                            <p class="p-large">Use the request buttons below to respond for the assests requests received in the system the tick is for approving request and x is for declining request.</p>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$row-> serial_no }}</td>
                                 ...
                                 <td>{{$row-> status }}</td>
                                <td><a class="nav-link page-scroll" style="float: left;margin-right: 4px;" title="Click here to approve the request" href="{{ route('asset_response') }}"
                                        onclick="event.preventDefault();
                                                     document.getElementById('{{$row->id }}').submit();"><i class="fa fa-check"></i></a> 

                                    <form id="{{$row->id}}" action="{{ route('asset_response') }}" method="POST" style="display: none;">
                                        @csrf
                                    ...
                                    <a class="nav-link page-scroll" style="float: left;margin-right: 4px;" title="Click here to decline the request" href="{{ route('asset_response') }}"
                                        onclick="event.preventDefault();
                                                     document.getElementById('{{$i }}').submit();"><i class="fa fa-ban"></i></a> 

                                    <form id="{{$i }}" action="{{ route('asset_response') }}" method="POST" style="display: none;">
                                        @csrf
                                        <input type="hidden" name="company_name" value="{{$row-> company_name }}">
                                        <input type="hidden" name="asset_name" value="{{$row-> asset_name }}">
                                        <input type="hidden" name="deny_status" value="denied">
                                        <input type="hidden" name="serial_no" value="{{$row-> serial_no }}">
                                        <input type="hidden" name="manager_first_name" value="{{Auth::user()->f_name }}">
                                        <input type="hidden" name="manager_last_name" value="{{Auth::user()->l_name }}">
                                        <input type="hidden" name="manager_email" value="{{Auth::user()->email }}">
